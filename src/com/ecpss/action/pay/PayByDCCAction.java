...
public class PayByDCCAction extends BaseAction {
    ...
    if(tradeType.equals("YY")){		
   	 ...
   	 msg2.setDATE_EXPIRATION(card.getExpiryDate());//14   有效期
...
//   	 msg2.setRETRIEVAL_REFERENCE_NUMBER("");//37
   	 ...
   	 msg2.setCurrency_Code_Foreign("970");//49 货币代码-----0810
    ...
    if(tradeType.equals("YX")){
    	 ...
    	 dcc3.setDATE_EXPIRATION(card.getExpiryDate());//14   有效期
    	 dcc3.setPOINT_OF_SERVICE_ENTRY_CODE("0012");//22 POS进入模式
    	 dcc3.setNETWORK_INTL_IDENTIFIER("0017");//24  收单商户号
         dcc3.setCARD_ACCEPTOR_TERMINAL_ID(trade.getVIPTerminalNo());//41  商户终端号
    	 dcc3.setCARD_ACCEPTOR_ID_CODE(tim.getMerchantNo());//42 商户编号 
