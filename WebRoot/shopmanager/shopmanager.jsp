...
<script language="JavaScript" src="../js/util.js"></script><head>
    ...
    function updateExpressReg(merid){
    	openWindow("../expressOpen.action?merid="+merid,'12');
    }
...
<%-- <input type="button" value="物流设置" class="button_08"  onclick="openWindow('../PaySystem/expressOpen.action','merid=<s:property value="#s.id"/>');"/> --%>
<input type="button" value="物流设置" class="button_08"  onclick="updateExpressReg('<s:property value="#s.id"/>');"/>
<input type="button" value="身份审核" class="button_10"  onclick="openWindow('../PaySystem/see.action','merid=<s:property value="#s.id"/>');"/>
<input type="button" class="button_09"  name="p" value="<s:if test="#s.isopen==1">关闭</s:if><s:else>开通</s:else>" onclick="openOrClose(<s:property value="#s.id"/>);"/>
</td>
                    </tr>
                    </s:iterator>
  </table>
